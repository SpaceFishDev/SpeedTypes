// Copyright (C) Liam Gatter 2022
// License details in license.md in root folder.
...
{
    append_node(&hash->elements, InitializeNode(element));
    size_t index_el = hash->count++;
    size_t index = HashString(element->key);
...
{
    size_t hash_key = HashString(key);
...
size_t hash_string(char* string)
{
    ...
    total = total / strlen(cpy) % (2*strlen(cpy));
    return total;
}
